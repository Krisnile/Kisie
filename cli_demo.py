...
DEFAULT_CKPT_PATH = '.\\kisie_1_1'

_WELCOME_MSG = '''\
Welcome to use Kisie model, type text to start chat, type :h to show command help.
(欢迎使用 Kisie 模型，输入内容即可进行对话，:h 显示命令帮助。)

Note: This demo is governed by the original license of Kisie.
...
(注：本演示受 Kisie 许可协议限制。我们强烈建议，用户不应传播及不应允许他人传播以下内容，包括但不限于仇恨言论、暴力、色情、欺诈相关的有害信息。)
'''
_HELP_MSG = '''\
Commands:
...
def main():
    ...
    while True:
        ...
        try:
            ...
            with open('.\\Dialogue_log\\dialog.md','a') as f:
                f.write(f"\n\ndialogue time: {time_string}")
                f.write(f"\nUser: {query}")
                f.write(f"\nKisie: {response}")
                f.close
        except KeyboardInterrupt:
